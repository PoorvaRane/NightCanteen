...
public class MenuReader {
	...
	private Scanner readMenu;
        private Scanner readMenu1;
	private BufferedWriter orderLogger;
        private BufferedWriter orderUpdate;
        private BufferedWriter FinalOrder;
        private BufferedWriter FinalUpdate;
        
	private FileWriter orderLoggerStream;
        private FileWriter orderUpdateStream;
        private FileWriter FinalOrderStream;
        private FileWriter FinalUpdateStream;
    ...
    MenuReader(String menutxt_ItemOrderstxt) {
	 ...
	 * We also list the total price and menu items. 
         * We throw an IO Exception as a checked exception
            * @param itemsOrdered
            * @param orderPrice
	...
	public void logOrder(ArrayList<MenuItem> itemsOrdered, BigDecimal orderPrice) throws IOException {
		...
		orderLogger = new BufferedWriter(orderLoggerStream);
                ...
                FinalOrderStream = new FileWriter(file,true);
		FinalOrder = new BufferedWriter(FinalOrderStream);
		java.util.Date orderDate = new java.util.Date();
                ufile = new File("update.txt");
		
                FinalUpdateStream = new FileWriter(ufile,true);
		FinalUpdate = new BufferedWriter(FinalUpdateStream);
		...
		orderLogger.write("Total Price is: Rs" + orderPrice + "\n\n");
                FinalOrder.write(orderPrice+"\n");
		orderLogger.close();
                FinalOrder.close();
                FinalUpdate.close();
        }
